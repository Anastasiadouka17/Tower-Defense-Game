...
public class BeeColony extends Frame {
    ...
    private static final int STINGYBEE_COST = 1;
    private static final int TANKYBEE_COST = 3;

    private static final int STINGYBEE_ATTACK = 1;
    ...
    private static Button stingyBeeBtn;
    ...
    private boolean spawnBee(Tile tileToSpawn) {
        ...
        switch (_selectedBeeType) {
            case busyBee:
                ...
                break;
            case stingyBee:
                System.out.println("Spawn stingy bee");
                if (_totalFood >= STINGYBEE_COST) {
                    UpdateFood(-STINGYBEE_COST);
                    bees.add(new StingyBee(tileToSpawn, STINGYBEE_ATTACK));
                    return true;
                }
                break;
            case tankyBee:
    ...
    private void createButtons(Panel buttonPanel) {
        ...
        // button for stingy bee
        stingyBeeBtn = new Button("Stingy Bee ($1)");
        stingyBeeBtn.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                //set selected bee type
                //in selection to true
                _selectedBeeType = EnumBeeType.stingyBee;
                _inSelection = true;
                UpdateMessage("You selected Stingy Bee");
            }
        });
